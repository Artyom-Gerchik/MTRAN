    ...
    public void GetTokens()
    {
        ...
        foreach (var symbol in Code)
        {
            ...
            else
            {
                ...
                {
                    ErrorMessage =
                        $"{Path} ({row}, {col - word.Length}): string constant not closing: {word}\n{info}";
                    IsError = true;
                    break;
                }
                ...
                {
                    ErrorMessage =
                        $"{Path} ({row}, {col - word.Length}): char constant not closing: {word}\n{info}";
                    IsError = true;
                    break;
                }
                ...
                {
                    if (VariableExists(word, environment, true) == string.Empty &&
                        !CurrentKeyWords.ContainsKey(word))
                    {
                        var type = Variable();
                        ...
                        else
                        {
                            ...
                            {
                                ErrorMessage =
                                    $"{Path} ({row}, {col - word.Length}): looks like bullshit: {word}\n{info}";
                                IsError = true;
                                break;
                            }
                        }
                    ...
                    else
                    {
                        ...
                        {
                            ErrorMessage =
                                $"{Path} ({row}, {col - word.Length}): variable duplicate: {word}\n{info}";
                            IsError = true;
                            break;
                        }
                    }
                ...
                else if (word.StartsWith("\'") && word.EndsWith("\'") && word.Length >= 2)
                {
                    if (word.Length >= 3)
                    {
                        ...
                        {
                            ErrorMessage =
                                $"{Path} ({row}, {col - word.Length}): bad char constant defenition: {word}\n{info}";
                            IsError = true;
                            break;
                        }
                    }
                ...
                else if (word != string.Empty)
                {
                    ...
                    {
                        ErrorMessage =
                            $"{Path} ({row}, {col - word.Length}): identifier name missing: {word}\n{info}";
                        IsError = true;
                        break;
                    }
                    else
                    {
                        ...
                        {
                            ErrorMessage =
                                $"{Path} ({row}, {col - word.Length}): undefined identifier: {word}\n{info}";
                            IsError = true;
                            break;
                        }
                    }
                    ...
                    if (Operations.ContainsKey($"{symbol}"))
                    {
                            ...
                            && $"{symbol}" == Tokens[^1].Identifier)
                        {
                            ...
                            Tokens.Add(new Token($"{symbol}{symbol}", "operation"));
                                     ...
                                     || Tokens[^1].Identifier == "*" || Tokens[^1].Identifier == "/" ||
                                     Tokens[^1].Identifier == "!"))
                        {
                            if (!CurrentOperations.ContainsKey($"{Tokens[^1].Identifier}{symbol}"))
                            {
                                ...
                                CurrentOperations[$"{Tokens[^1].Identifier}"] =
